...
export const createTeamWithCallbacks = (team: Team, prefix: string): Team => {
  ...
  return {
    ...
    squad: {
      ...
      cdm: team.squad.cdm?.map((p: Player, i: number) => createPlayerWithCallback(p, `CDM${i + 1}`)),
      cm: team.squad.cm?.map((p: Player, i: number) => createPlayerWithCallback(p, `CM${i + 1}`)),
      cam: team.squad.cam?.map((p: Player, i: number) => createPlayerWithCallback(p, `CAM${i + 1}`)),
