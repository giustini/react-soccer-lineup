...
Player marker diameter options.
...
By default player names render at their full width and are never truncated. Setting `nameOverflow` caps a name to its player column width — so it never overlaps neighbouring players — and chooses how the overflow is truncated.
